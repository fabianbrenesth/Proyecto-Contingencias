...
simulacion.hombres <- function(hombres, primas.por.edad, suma.asegurada4, suma.asegurada5, suma.asegurada6){
  ...
  for(i in 2:length(matriz[,1])){
    ...
    suma.asegurada5.ajustado <- suma.asegurada4 * ((1.03)^(i-1))
    suma.asegurada6.ajustado <- suma.asegurada4 * ((1.03)^(i-1))
...
for (i in 1:cant.simulaciones){
  ...
  años <- 1:103  # Años correspondientes a los flujos

  vp_ingresos.m <- ingresos.m / (1 + interes[años])^años
  vp_egresos.m <- egresos.m / (1 + interes[años])^años
  
  vp_ingresos.h <- ingresos.h / (1 + interes[años])^años
  vp_egresos.h <- egresos.h / (1 + interes[años])^años
